...
* sda: se usa para identificar dispositivos que utilizan SCSI (Small Computer System Interface), "sd" también incluye SATA o puertos USB. SCSI es un estándar para la transferencia de datos.
...
Al momento de ejecutarlo pide la contraseña y una vez dada, se ingresa a root (súper usuario), donde se tienen todos los permisos. En la siguiente imagen, se puede ver aplicado lo anterior, el prompt cambia a "#" y dice que nos encontramos en root.

![Imagen 02](/ImagesREADME/2.png)

Está es la razón por la cual en los siguientes puntos al inicio de cada instrucción se va a utilizar el siguiente comando, con el fin de tener los permisos de súper usuario:
```bash
sudo
```
...
- umask = 0: quiere decir que se otorguen los permisos anteriores para todos los usuarios
...
Dicho comando da como resultado todos los dispositivos de bloque conectados, proporciona información de los dispositivos como nombre y "MOUNTPOINT", la cual es una sección que tiene como parámetro la ruta donde se ha montado el dispositivo. Si no hay ningún parámetro en la sección significa que el dispositivo no está montado.
...
Una vez que se ingresa a fdisk, se utiliza la letra "d" para indicarle que queremos eliminar una partición, nos pide el número de partición que queremos borrar y se elimina, y así sucesivamente. Estos cambios son realizados en memoria, para escribirlos se utiliza la letra "w" y así se modifica la tabla de particiones de manera definitiva.
